...
def main(conf):
    ...
    audiomodel.to(model_device)
    videomodel.to(model_device)

    # Randomly choose the indexes of sentences to save.
    torch.no_grad().__enter__()
    for idx in range(1, 3):
        spk, sr = sf.read("test_videos/interview_visualvoice/interview.wav", dtype="float32")
        mouth = get_preprocessing_pipelines()["val"](np.load("test_videos/interview_visualvoice/mouthroi/speaker{}.npz".format(idx))["data"])
        key = "spk{}".format(idx)
